...
interface BaseRepositoryPrMergedTransform {
  merged: boolean;
  mergedCount: number;
}

export class BaseRepositoryPrMerged extends BaseAchievement<
  PullRequestClosedEvent,
  BaseRepositoryPrMergedTransform
> {
  ...
  async transform(): Promise<BaseRepositoryPrMergedTransform> {
    ...
    return {
      ...
      mergedCount: this.repository.gitHubRepositoryCounts[0].pullRequestsMerged,
    };
  }

  checkQualification(mergeInfo: BaseRepositoryPrMergedTransform): boolean {
